...
public class StockInformationCrawler {
    ...
    private StockInformationCrawler() {

    }
    
    private static final Logger LOG = Logger.getLogger(StockInformationCrawler.class);
    ...
    private static Document getStockDocument(String url) {
        ...
        try {
            ...
            LOG.error("Failed to connect to stockpage", e);
        }
    ...
    public static void marshallDetail(StockBean stock) {
        ...
        for (int i = 0; i < detailElement.children().size(); i++) {
            ...
            if ("净利润增长率：".equals(detailElement.child(i).text())) {
                
                String value = detailElement.child(i + 1).text();
//                /System.out.println(value);
                stock.setZzl(Double.parseDouble(StringUtils.remove(value, "%")));
            }
        }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 2600; i < 2750; i++) {
            ...
            try {
                ...
                LOG.error("Exception happens when marshell returned html", e);
                continue;
            }
            stocks.add(stock);
        }
